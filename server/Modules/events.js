...
import {db_get_block_internal_commands, db_get_block_trans} from "./db.js";
...
export const on_canonical_block = async (data) => {
    ...
    let tx = await db_get_block_trans(block_id)
    for(let row of tx) {
    ...
    // Process coinbase
    tx = await db_get_block_internal_commands(block_id)
    for(let row of tx) {
        const {receiver_id, receiver_key} = row
        const receiver_balance = await ql_get_account_balance(receiver_key)
        if (receiver_balance) {
            query(`
                insert into balances (public_key_id, block_id, total, liquid, locked, unknown)
