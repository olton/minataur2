...
const updateDaemonStatus = (data) => {
    ...
    $("#consensus_time_best_tip").html(`epoch: ${!status.consensus_time_best_tip ? 0 : Math.floor(status.consensus_time_best_tip.slot_number/status.consensus_time_best_tip.slots_per_epoch)}, slot: ${!status.consensus_time_best_tip ? 0 : status.consensus_time_best_tip.slot_number - status.consensus_time_best_tip.slots_per_epoch}`)
    $("#consensus_time_now").html(`epoch: ${!status.consensus_time_now ? 0 : Math.floor(status.consensus_time_now.slot_number/status.consensus_time_now.slots_per_epoch)}, slot: ${!status.consensus_time_now ? 0 : status.consensus_time_now.slot_number - status.consensus_time_now.slots_per_epoch}`)
